...
def check_url_status(link):
    try:
        response = requests.head(link, allow_redirects=True, timeout=5)  # HEAD request for faster response
        if response.status_code == 200:
            return True
        else:
            print(f"{link} -> Invalid Link (Status Code: {response.status_code})")
            return False
    except requests.RequestException:
        print(f"{link} -> Invalid Link (Request Failed)")
        return False
